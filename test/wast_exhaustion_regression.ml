...
let require_runtime_shape () =
  ...
  List.iter
    ...
    ["command.exhaustion(1,2,action.invoke(1,";
     ...
     "rel.step(config.sym(state.sym(S,rec.frame(LOCALS,CURRENT)),BODY))=>config.sym(C2,INSTRS)/\\_>_(activeFrameDepth(INSTRS),REQUIRED)=true.";
     "=>script.ready(S,ENV,CMDS)ifrel.step(";
     "rel.step(C)=>config.sym(C2,INSTRS)/\\_<=_(activeFrameDepth(INSTRS),REQUIRED)=true.";

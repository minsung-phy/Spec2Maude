...
let inputs_may_overlap _index left right =
  List.length left = List.length right
  && List.for_all2 (may_overlap [("_ _", "eps")]) left right
...
let lower_execution_rules ?request_output
...
let translate_execution ?request_output

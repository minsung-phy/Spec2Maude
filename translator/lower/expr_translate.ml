...
let lower_case_impl ~lower_value ~category_guards_for_term ctx env origin exp mixop arg_exp =
  ...
  let terms = List.filter_map (fun result -> result.term) arg_results in
  if List.length terms = List.length arg_results then
    let arity = List.length terms in
    match exp.note.it, terms with
    | VarT (id, _), _ ->
      ignore id;
      (match emitted_typcase_constructor_lookup ctx exp.note mixop arity with
      | Constructor_found constructor ->
...
let lower_uncase_projection_impl
  ...
  if index < 0 || index >= arity then
    ...
    match emitted_typcase_constructor_resolution_lookup ctx scrutinee.note mixop arity with
    | `Found resolution ->
...
let lower_call_impl ~lower_value ctx env origin exp id args =
  ...
  let guards = List.concat (List.map (fun result -> result.guards) results) in
  let resolution_diagnostics, op_name =
    match resolution with
    | Analysis.Function_graph.Plain_call -> [], Some (Naming.definition_op target_id)
    | Analysis.Function_graph.Specialized_call specialization ->
      if Analysis.Function_graph.has_specialization graph specialization then
        [], Some (Naming.specialized_definition_op target_id specialization.key_components)
      else
        [ unsupported
  ...
  if List.exists Diagnostics.is_fatal diagnostics then
    ...
    match op_name with
    | Some op_name -> { term = Some (app op_name terms); guards; diagnostics }
...
let pattern_result_to_expr (result : Pattern_translate.result) =
  { term = result.term; guards = result.guards; diagnostics = result.diagnostics }
...
let lower_pattern_raw callbacks ctx env origin exp =
  ...
  in
  Pattern_translate.lower ctx pattern_callbacks origin exp
...
let rec lower_value ctx env origin exp =
  ...
  | IdxE (base, index) ->
    lower_index ctx env origin base index
...
and lower_numeric_subtyping ctx env origin exp inner source_typ target_typ =
  Expr_bool_numeric.lower_numeric_subtyping expr_bool_numeric_callbacks ctx env origin exp inner source_typ target_typ
...
and lower_uncase ctx env origin exp scrutinee mixop =
  let arity = List.length (typ_components exp.note) in
  if arity = 1 then
...
and lower_index ctx env origin base index =
  Expr_record.lower_index expr_record_callbacks ctx env origin base index
...
let lower_pattern_with_bindings ctx env origin exp =
  lower_pattern_raw pattern_callbacks ctx env origin exp
  |> pattern_result_to_expr_pattern

let lower_pattern ctx env origin exp =
  lower_pattern_raw pattern_callbacks ctx env origin exp
  |> pattern_result_to_expr

...
type output = Reld_result.output =
    ...
    | None ->
      one_diagnostic
        (skipped
           ...
           ~reason:
             ("static validation predicate relation is discharged because the runtime starts from an initial configuration checked by the official validator; structural relation classification is "
              ^ relation_kind_text)
           ~suggestion:

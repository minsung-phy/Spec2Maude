...
let result_var origin rel_id output_sort =
  Premise_rule_support.fresh_result_var
    ~fallback:"DET"
    ~label:"deterministic-false"
    origin
    rel_id
    output_sort

let alternative_rule helper_name origin label lhs rhs result_var input_guards entry =
...
let materialize
  ...
  | Supported_case { shape; inputs; family } ->
    let input_terms, input_guards, input_diagnostics =
      Premise_rule_support.lower_input_values ctx env origin inputs
    in
    ...
    | Some input_terms, Some output_sort ->
      let result_var = result_var origin rel_id output_sort in

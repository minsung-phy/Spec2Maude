...
let find_native_child_category ctx target_id signature =
  native_variant_case_signatures ctx target_id
  |> List.find_map (fun (child_id, signatures) ->
    if List.exists (same_typcase_signature signature) signatures then
...
let inherited_category_cases ctx target_id cases =
  cases
  |> List.mapi (fun index typcase ->
    if typcase_is_inherited_from_other_region typcase then
      let signature = signature_of_typcase typcase in
      match find_native_child_category ctx target_id signature with
      | Some (child_id, child_signatures) ->
        Some

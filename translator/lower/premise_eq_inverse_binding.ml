...
let lower_inverse_arg_for_param ctx env ~bound_vars origin param arg =
  match param, arg.it with
  | Analysis.Function_graph.Runtime_exp, ExpA exp ->
    lower_inverse_arg ctx env ~bound_vars origin exp
  | Static_typ, TypA typ ->
    let result =
...
let collect_inverse_args ctx env ~bound_vars origin exp params args =
  let rec loop items guards diagnostics params args =
    ...
    | param :: params, source_arg :: args ->
      (match lower_inverse_arg_for_param ctx env ~bound_vars origin param source_arg with
...
let direct_runtime_target ctx env ~bound_vars origin exp =
  match unbound_env_var_binding env ~bound_vars exp with
  | Some (id, binding) ->
    if Condition_closure.is_match_pattern binding.term then
      Ok (Some id)
...
let direct_runtime_targets ctx env ~bound_vars origin params args =
  let step acc item =
    match acc with
    ...
    | Ok targets ->
      ...
      | Analysis.Function_graph.Runtime_exp, { it = ExpA exp; _ } ->
        (match direct_runtime_target ctx env ~bound_vars origin exp with
        | Ok None -> Ok targets
...
let inverse_original_terms args =
  ...
  |> List.map (function
    ...
    | { arg = Inverse_target (_id, binding); _ } -> binding.Expr_translate.term)
...
let inverse_target_admissible
  ...
  | Some sort ->
    sort_name sort = sort_name target_binding.Expr_translate.sort
  | None -> false
...
let inverse_definition_implemented
  ...
  ||
  match Builtin_spec.status_of_source_id inverse_definition.id with
  | Builtin_types.Implemented -> true
  | Builtin_types.Obligation -> false
...
let lower ctx env ~bound_vars origin exp call_exp known_exp =
  ...
  | CallE (id, args) ->
    ...
    (match
     ...
     with
    | Some definition, Some inverse_id when List.length definition.params = List.length args ->
      (match direct_runtime_targets ctx env ~bound_vars origin definition.params args with
      | Error diagnostics ->
      ...
      | Ok [ _target ] ->
        ...
        | Some inverse_definition ->
          if not (inverse_definition_implemented inverse_definition) then
            Some
              { (empty_with_env ~bound_vars env) with
                diagnostics =
                  [ unsupported_exp
          ...
          (match collect_inverse_args ctx env ~bound_vars origin exp definition.params args with
          | Error diagnostics ->
            Some { (empty_with_env ~bound_vars env) with diagnostics }
            ...
            | [ (target_id_text, target_binding) ] ->
              ...
              | Some known_term ->
                ...
                if List.length inverse_definition.params <> List.length inverse_terms then
                  ...
                  let inverse_call =
                    app (Naming.definition_op inverse_id_phrase) inverse_terms
                  in
                  let original_call =
                    app

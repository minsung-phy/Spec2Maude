...
let helper_names env source_ids terms =
  ...
  | RulePr (rel_id, [], _, exp) ->
    ...
    | [ component_index, (source_exp, index_source_id) ], Some _ ->
      ...
      | Some source_term ->
        ...
        in
        let names = helper_names env source_ids [ history ] in
        let source_captures =
...
let forall_edge ctx item relations origin identity env history premise body_plan =
  ...
  let materialize body generators sources =
    ...
    in
    let names = helper_names env (generator_ids @ capture_ids) [ history ] in
    let captures =
      capture_ids
            ...
            let false_result =
              ...
              | Decide ->
                (match
                 ...
                 with
                | Ok (_, alternatives, diagnostics) -> Ok (alternatives, diagnostics)
                | Error blockers ->
                  Error
                    (List.map
                       (Runtime_truth_total_equality.diagnostic ctx) blockers))
            in
...
let worklist_pattern_certificate ctx item relations =
  ...
  surface_pattern_certificate ctx
    (helper_surface item @ relation_statements @ seed_statements)
    ...
    let rules =
      ...
      |> List.map (fun (rule_index, rule) ->
        let origin, declarations, bind_diags, head =
          lower_head ctx item relation rule_index rule
        in
...
let target_chain_edge
  ...
  | Some (known, [ target_term ]) ->
    ...
    | Some (source_id, binding) ->
      let names = helper_names head_env [ source_id ] (head_terms @ [ history ]) in
      let witness_name =
      ...
      if Premise_result.rule_conditions prefix_result <> [] then
        ...
        | Some source_term ->
          ...
          if unbound_source <> [] then
            ...
            let names =
              helper_names source_env (index_source_id :: successor_ids)
                (terms @ [ source_term ])
            in
...
let domain_candidate ctx item env origin source_echo index = function
  ...
  | Indexed_domain
    ...
    | Some source_term, [] ->
      ...
      in
      let names =
        helper_names env (index_source_id :: capture_ids) [ source_term ]
      in
      let capture_candidates =
    ...
    | Some (prefix, [ left; right ]) ->
      ...
      if List.exists (function Blocked _ -> true | Materialized _ -> false) producers then
        ...
        let invocation =
          Runtime_truth_worklist_helper.invocation ~helper_name:item.name item.request
        in
        let names = helper_names head_env [] (head_terms @ [ history ]) in
        let captures, names =
...
let lower ctx item relations relation index rule =
  ...
  | Some terms ->
    ...
    if not children.complete then
      ...
      let lhs = App (prove_op item relation.id, terms @ [ history ]) in

...
let finite_helper_surface ctx item (plan : Runtime_witness_domain.t) rules =
  ...
  let rule_candidates, _candidate_diagnostics =
    Runtime_truth_candidate_domain.rule_head_candidates
      ctx
      ~parent:item.origin
      ~input_count:(List.length item.request.input_sorts)
      plan
      rules
  in
  let candidate_terms =
    Runtime_truth_candidate_domain.query_candidates plan input_vars
    @ rule_candidates
    |> Runtime_truth_candidate_domain.dedup_terms
  ...
  @ [ generated
        ...
        (Maude_ir.eq
           ...
           (Runtime_truth_candidate_domain.terminal_sequence candidate_terms))
    ; generated
        item.name
        item.origin
        (Maude_ir.crl
...
let lower_seed_rule_with_surface
  ...
  | Some components ->
    match
      Runtime_truth_candidate_domain.split_exps
        target.Runtime_witness_proof.prefix_arity
        components
    with
    | None ->

...
let target_numeric_var names env ~bound_vars exp =
  match unbound_direct_var env ~bound_vars exp with
  | None -> None
  | Some id ->
    (match typed_var_for_exp names id exp with
    | Some (_term, binding)
      when List.mem (sort_name binding.Expr_env.sort) [ "Nat"; "Int" ] ->
      Some (id.it, binding)
    | Some _ | None -> None)

let multiplication_target names ctx env ~bound_vars origin product_exp =
  ...
  let candidate target_exp factor_exp product =
    match target_numeric_var names env ~bound_vars target_exp with
    | None -> None
    | Some (id, binding) ->
...
let lower names ctx env ~bound_vars origin exp product_exp known_exp =
  match multiplication_target names ctx env ~bound_vars origin product_exp with
  | None -> None
    ...
    | Some factor_term, Some known_term ->
      ...
      | Some bound_after_prefix ->
        ...
        if not (vars_subset needed bound_after_prefix) then
          ...
          in
          let env_after =
            Expr_env.add env target_id target_binding
          in
          Some
            (with_conditions
               ctx

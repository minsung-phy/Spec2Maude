...
let lower_unary_projection ctx env ~bound_vars origin _exp projection_exp value_exp =
  ...
  | ProjE (({ it = UncaseE (scrutinee_exp, _mixop); _ } as _uncase_exp), 0) ->
    ...
    | VarE scrutinee_id ->
      (match Expr_env.find env scrutinee_id.it with
...
let lower_direct_binding_value ctx env origin target_binding value_exp =
  let value_result =
    if sort_name target_binding.Expr_env.sort = "SpectecTerminals" then
  ...
  match value_result.term with
  | Some _ -> value_result
  | None when is_raw_numeric_sort target_binding.Expr_env.sort ->
    let numeric_result =
...
let lower_direct_var names ctx env ~bound_vars origin _exp target_exp value_exp =
  let target_exp, value_exp =
    projection_binding_sides target_exp value_exp
  in
  match unbound_var_binding names env ~bound_vars target_exp with
  | None -> None

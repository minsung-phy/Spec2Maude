...
let with_conditions
    ...
    ctx env bound_vars conditions diagnostics =
  ...
  { (Premise_result.empty_with_env
       ~bound_vars:
         (Condition_closure.conditions_bound_vars
            ~constructor_op:pattern_certificate bound_vars conditions)
       env)
    with
...
let typed_var_for_exp names id exp =
  ...
  | Some sort ->
    ...
    in
    Some (term, { Expr_env.term; sort; typ = exp.note })

let same_sort left right =
  sort_name left = sort_name right

let unbound_var_binding names env ~bound_vars exp =
  match exp.it with
  | VarE id ->
    let typed_binding =
      typed_var_for_exp names id exp |> Option.map snd
    in
    (match Expr_env.find env id.it with
    | Some binding when not (binding_is_bound bound_vars binding) ->
      (match typed_binding with
      | Some typed when not (same_sort typed.Expr_env.sort binding.sort) ->
        Some (id.it, typed)

...
let search_sort name =
  Maude_ir.sort ("RuntimeSearch" ^ name ^ "Conf")

let loop_op name =
  "runtimeSearchLoop" ^ name

let candidates_op name =
  "runtimeSearchCandidates" ^ name

let candidate_op name =
  "runtimeSearchCandidate" ^ name

let visited_key_op name =
  "runtimeSearchVisitedKey" ^ name
...
let helper_surface item =
  ...
  let search_op = Runtime_search_helper.search_op ~helper_name:item.name in
  let hit_op = Runtime_search_helper.hit_op ~helper_name:item.name in
  ...
  ; generated
      ...
      (Maude_ir.op
         hit_op
         [ Maude_ir.sort_ref request.witness_sort ]
         result_sort
         ~attrs:[ Maude_ir.Ctor ])
  ]
...
let lower_patterns ctx origin exps =
  let rec loop env terms guards bindings diagnostics = function
    | [] ->
      Some (List.rev terms),
      env,
      List.rev guards,
    ...
    | exp :: exps ->
      ...
      in
      let result = Expr_translate.lower_pattern_with_bindings ctx env exp_origin exp in
      let env =
  ...
  if not (String.equal rule.relation_id request.rel_id) then
    ...
    | Some components ->
      ...
      | Some (left, witness_exp, right) ->
        ...
        let terms_opt, env, head_guards, _bindings, head_diags =
        ...
        | Some terms ->
          ...
          | [ witness_term ] ->
            ...
            if premise_result.has_else then
            ...
            else if premise_result.runtime_search_requests <> [] then
              Error
                (diagnostics
                 @ [ unsupported
                       ctx
              ...
              let conditions =
                ...
                @ List.map (fun condition -> EqCondition condition) premise_result.eq_conditions

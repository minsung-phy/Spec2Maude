...
2. step-pure test
```
rew [1] in WASM-FIB-BS : step-pure(NOP) .
search [1] in WASM-FIB-BS : step-pure(NOP) =>* eps .
```
...
search [1] in WASM-FIB-BS : steps(fib-config(i32v(5))) =>* (fib-store, empty-frame, CONST(I32, 5) .
```
...
red in WASM-FIB-BS-PROPS :
    modelCheck(steps(fib-config(i32v(0))), [](done -> result-is(5))) . --- 끝난 상태라면 결과가 0이어야 함

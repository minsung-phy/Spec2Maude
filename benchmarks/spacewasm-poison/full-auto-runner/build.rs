...
fn corrupt_code_vector_count(mut wasm: Vec<u8>) -> Vec<u8> {
    ...
    while at < wasm.len() {
        ...
        if section_id == 10 {
            // The active element segment has already changed the imported
            // table when SpaceWasm later observes this malformed code vector.
...
fn main() {
    ...
    let attacker = corrupt_code_vector_count(wat::parse_file("../attacker.wat").unwrap());
    let future = wat::parse_file("../future.wat").unwrap();

    fs::write(out.join("provider.wasm"), provider).unwrap();
    fs::write(out.join("attacker-invalid.wasm"), attacker).unwrap();

...
docker run --rm \
  ...
  bash -lc '
    ...
    emcc /src/callback_bad_side.c -O0 \
      -sSIDE_MODULE=2 \
      -Wl,--export=failed_plugin_callback \
      -o /out/libcallback-bad.wasm
    ...
    compile_main callback_main.c /out/callback-production.js
    ...
    node callback-production.js > callback-production.log 2>&1 || true
    ...
    compile_main callback_main.c /out/callback-fixed.js
    compile_main symbol_poison_main.c /out/symbol-fixed.js
    node main-fixed.js > fixed.log 2>&1 || true
    node callback-fixed.js > callback-fixed.log 2>&1 || true
...
# Finding 2a: the failed side module has already grown the shared table and its
...
# Finding 2b: the same lifecycle leak is reachable through an ordinary C
# function pointer. The failed plugin registers its own callback in the host,
# then traps. dlopen returns NULL, but the host invokes the failed plugin twice
# and observes its private counter. No internal JS table API is used here.
for log in callback-production callback-fixed; do
  grep -q '^callback_before=7$' "$out_dir/${log}.log"
  grep -q '^callback_load_handle_nonnull=0$' "$out_dir/${log}.log"
  grep -q '^callback_after_failure_1=71$' "$out_dir/${log}.log"
  grep -q '^callback_after_failure_2=72$' "$out_dir/${log}.log"
done

# Cache eviction fixes the false-success handle but cannot revoke either form
...
# Documented Emscripten semantic gap: RTLD_NOW is accepted for a consumer whose
# required symbol is not available. dlopen and dlsym both report success; the
# first call escapes the Wasm boundary as a JavaScript TypeError. Failed-DSO
# cache cleanup does not affect this independent behavior.
...
# native RTLD_NOW failure boundary: the unrelated consumer is reported loaded
# and only crashes when its unresolved import is called.
...

{
  ...
  echo
  echo '[Finding 2a: residual Wasm table capability]'
  echo 'failed_dlopen_residual_results=41,42'
  echo 'cache_cleanup_residual_results=41,42'
  echo
  echo '[Finding 2b: host-visible C callback from a failed plugin]'
  grep '^callback_' "$out_dir/callback-production.log"
  echo 'failed_plugin_callback_status=EXECUTABLE_AFTER_DLOPEN_NULL'
  echo
  echo '[Documented RTLD_NOW semantic gap]'

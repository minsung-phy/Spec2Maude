...
let sequence_sort element_sort iter =
  let list_sort, nonempty_sort =
    match element_sort with
    | "Val" -> Some "ValList", Some "NeValList"
    | "Instr" -> Some "InstrList", Some "NeInstrList"
    | "Ref" -> Some "RefList", Some "NeRefList"
    | _ -> None, None
  in
  match iter, list_sort, nonempty_sort with
  | List1, _, Some sort -> sort
  | (List | ListN _), Some sort, _ -> sort
  | Opt, _, _ | _, None, _ | _, _, None -> "SpectecTerminals"
...
let rec representation_sort env definitions seen typ =
  match typ.it with
  | VarT (id, _) ->
      begin match annotated_sort id.it with
      | Some sort -> sort
      | None -> representation_sort_reduced env definitions seen typ
      end
  | IterT (element, iter) ->
      sequence_sort (representation_sort env definitions seen element) iter
  | _ -> representation_sort_reduced env definitions seen typ

and representation_sort_reduced env definitions seen typ =
  let typ = Il.Eval.reduce_typ env typ in
  match typ.it with
  | VarT (id, _) ->
      begin match annotated_sort id.it with
      | Some sort -> sort
      | None when not (List.mem id.it seen) ->

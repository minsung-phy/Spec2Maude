...
let as_sequence_element index typ term =
  match Prescan.sort_of_typ index typ with
  | "ValList" | "NeValList" -> app "valSeq" [term]
  | "InstrList" | "NeInstrList" -> app "instrSeq" [term]
  | "RefList" | "NeRefList" -> app "refSeq" [term]
  | "SpectecTerminals" -> app "seq" [term]
  | _ -> term

let empty_for_sort = function
  | "ValList" | "NeValList" -> Const "valNil"
  | "InstrList" | "NeInstrList" -> Const "instrNil"
  | "RefList" | "NeRefList" -> Const "refNil"
  | _ -> Const "eps"

let concat_for_sort sort left right =
  match sort with
  | "ValList" | "NeValList" -> app "valConcat" [left; right]
  | "InstrList" | "NeInstrList" -> app "instrConcat" [left; right]
  | "RefList" | "NeRefList" -> app "refConcat" [left; right]
  | _ -> app "_ _" [left; right]

let empty_for_typ index typ =
  empty_for_sort (Prescan.sort_of_typ index typ)

let concat_for_typ index typ left right =
  concat_for_sort (Prescan.sort_of_typ index typ) left right

let iterated_typ typ iter =
  IterT (typ, iter) $ typ.at

let result_sort index body iter =
  Prescan.sort_of_typ index (iterated_typ body.note iter)

let list_tail_typ typ =
  match typ.it with
  | IterT (element, List1) -> IterT (element, List) $ typ.at
  | _ -> typ

let list_tail_sort index typ =
  Prescan.sort_of_typ index (list_tail_typ typ)

let tail_result_sort index body iter =
  match iter with
  | List1 -> result_sort index body List
  | Opt | List | ListN _ -> result_sort index body iter


(* Type iteration *)

let length_for_sort sort value =
  let name =
    match sort with
    | "ValList" | "NeValList" -> "valSize"
    | "InstrList" | "NeInstrList" -> "instrSize"
    | "RefList" | "NeRefList" -> "refSize"
    | _ -> "len"
  in
  app name [value]
...
let translate_conditions translate_count value element_type collection_sort iter =
  let length = length_for_sort collection_sort value in
  let checks =
    match collection_sort with
    | "ValList" | "NeValList"
    | "InstrList" | "NeInstrList"
    | "RefList" | "NeRefList" -> []
    | _ -> [typecheck value element_type]
  in
  match iter with
  | Opt ->
      checks @ [BoolCond (app "_<=_" [length; Const "1"])]
  | List ->
      checks
  | List1 ->
      checks @ [BoolCond (app "_<_" [Const "0"; length])]
  | ListN (count, _) ->
      checks @ [EqCond (length, translate_count count)]
...
let tail_variable index (id, source) =
  generated_variable
    (String.uppercase_ascii id.it ^ "S")
    (list_tail_sort index source.note)
...
let tail_helper_domain source_index captures count index generators =
  List.map (fun (variable : variable) -> variable.sort) captures
  @ List.map
      (fun (variable : variable) -> variable.sort)
      (Option.to_list count)
  @ List.map
      ...
      (Option.to_list index)
  @ List.map
      (fun (_, source) -> list_tail_sort source_index source.note)
      generators
...
let empty_arguments source_index captures count index generators =
  ...
  @ List.map
      (fun (_, source) -> empty_for_typ source_index source.note)
      generators
...
let source_head index generator head =
  term_of_variable head
  |> as_sequence_element index (generator_element generator)

let source_arguments index iter generators heads tails =
  match iter with
  | Opt ->
      List.map2
        (fun generator head -> app "_?" [source_head index generator head])
        generators heads
  | List | List1 | ListN _ ->
      List.map2
        (fun (generator, head) tail ->
          concat_for_typ index (snd generator).note
...
let step_arguments source_index iter captures count index generators heads tails =
  ...
  @ terms_of_variables (Option.to_list index)
  @ source_arguments source_index iter generators heads tails
...
let translate_identity_pattern index translate_pattern translate_conditions
  ...
  | Some source ->
      ...
      | Some (term, guards) ->
          ...
          in
          let collection_sort = result_sort index body iter in
          let subject_length = length_for_sort collection_sort term in
          let cardinality =
            match iter with
            | Opt ->
                Some [BoolCond (app "_<=_" [subject_length; Const "1"])]
            | List -> Some []
            | List1 ->
                Some [BoolCond (app "_<_" [Const "0"; subject_length])]
            | ListN (count, _) ->
                begin match translate_pattern count with
                | Some (count, count_guards) ->
                    Some (MatchCond (count, subject_length) :: count_guards)
                | None -> None
                end
          in
          Option.map
...
let column_term index generators columns =
  ...
  | _, _ ->
      List.map2
        (fun (_, source) column ->
          as_sequence_element index source.note column)
        generators columns
      |> sequence
...
let translate_projector_statements index translate_pattern can_bind_body bind_body
  ...
  then [] else
    ...
    let subject_sort = result_sort index body iter in
    let tails =
      generators
      |> List.mapi (fun position _ ->
           generated_variable
             ("PROJECT-COLUMN-" ^ string_of_int (position + 1))
             (list_tail_sort index (snd (List.nth generators position)).note))
    in
    let subject_tail_sort = tail_result_sort index body iter in
    let subject_tail =
      generated_variable "PROJECT-REST" subject_tail_sort
    in
    let declaration name iter domain_subject_sort codomain =
      OpDecl
        { name
        ; domain = projector_domain captures (count_variable iter)
                     (index_variable index iter) domain_subject_sort
        ; codomain
        ; arrow = Partial
        ; attrs = []
        }
    ...
    let column_sort =
      ...
      | [(_, source)] -> Prescan.sort_of_typ index source.note
      | _ -> "SpectecTerminal"
    in
    let tail_column_sort =
      match generators with
      | [(_, source)] -> list_tail_sort index source.note
      | _ -> "SpectecTerminal"
    in
    ...
    let empty =
      column_term index generators
        (List.map
           (fun (_, source) -> empty_for_typ index source.note)
           generators)
    in
    ...
    let subject =
      concat_for_sort subject_sort element (term_of_variable subject_tail)
    in
    ...
    match iter with
    | Opt ->
        [ declaration name Opt subject_sort column_sort
        ; Eq (call name None None (empty_for_sort subject_sort), empty, [])
        ; let left = call name None None (app "_?" [element]) in
          let right =
            column_term index generators
              (List.map2
                 (fun generator head ->
                   app "_?" [source_head index generator head])
                 generators heads)
          in
    ...
    | List ->
        [ declaration name List subject_sort column_sort
        ; Eq (call name None None (empty_for_sort subject_sort), empty, [])
    ...
    | List1 ->
        ...
        in
        [ declaration name List subject_sort column_sort
        ; declaration tail_name List subject_tail_sort tail_column_sort
        ; first
        ; Eq (call tail_name None None (empty_for_sort subject_sort), empty, [])
    ...
    | ListN _ ->
        ...
        in
        [ declaration name iter subject_sort column_sort
        ; Eq
            ( call name (Some (Const "0")) current_index
                (empty_for_sort subject_sort)
            , empty, [])
        ; recursive name
...
let pattern_count translate_pattern translate_exp known subject_sort subject iter =
  let length = length_for_sort subject_sort subject in
  match iter with
  ...
  | ListN (({it = VarE _; _} as count), _) ->
      ...
      |> Option.map (fun (pattern, guards) ->
           length,
           MatchCond (pattern, length) :: guards)
  | ListN _ -> None
  | Opt | List | List1 -> Some (Const "0", [])

let identity_cardinality translate_exp known subject_sort subject iter =
  let length = length_for_sort subject_sort subject in
  match iter with
  | Opt ->
      [BoolCond (app "_<=_" [length; Const "1"])]
  | List -> []
  | List1 ->
      [BoolCond (app "_<_" [Const "0"; length])]
  | ListN (count, _) when known count ->
      [EqCond (length, translate_exp count)]
  | ListN _ -> []
  ...
  | Some (source_patterns, source_guards) ->
      ...
      | Some _, [source_pattern] ->
          begin match pattern_count translate_source_pattern translate_exp
                        known subject_sort subject iter with
          | None -> None
              ...
              Some
                ( count_conditions
                  ...
                  @ identity_requirement_guards index translate_conditions
                      subject iter body
                  @ identity_cardinality translate_exp known subject_sort subject iter)
          end
      | None, _ when
      ...
      let forward =
        ...
        | (Opt | List | List1 | ListN _), _ ->
            ...
            let declaration =
              OpDecl
                { name
                ; domain = helper_domain index captures count iter_index generators
                ; codomain = result_sort index body iter
                ; arrow = Partial
                ; attrs = []
                }
            in
            let body_term =
              translate_exp body |> as_sequence_element index body.note
            in
            let output_sort = result_sort index body iter in
            let step_result =
              match iter with
              | Opt ->
                  app "_?" [body_term]
              | List | List1 | ListN _ ->
                  concat_for_sort output_sort body_term
            ...
            | List1 ->
                ...
                let tail_declaration =
                  OpDecl
                    { name = tail_name
                    ; domain =
                        tail_helper_domain index captures count iter_index generators
                    ; codomain = tail_result_sort index body iter
                    ; arrow = Partial
                    ; attrs = []
                    }
                in
                    ...
                    ( tail_call
                        (terms_of_variables captures
                         @ List.map
                             (fun (_, source) -> empty_for_typ index source.note)
                             generators)
                    , empty_for_sort (tail_result_sort index body iter)
                    , []
                    )
                in
                let tail_step =
            ...
            | Opt | List | ListN _ ->
                let base =
                  Eq
                    ( call (empty_arguments index captures count iter_index generators)
                    , empty_for_sort output_sort
                    , []
                    )
                in
...
let translate_premise_statements index translate_body
  ...
  let name, tail_name, generators, output, codomain, without =
    ...
    | Collect position ->
        ...
        Prescan.sort_of_typ index (snd generator).note, Some position
  in
  let captures =
    translate_captures index iteration.Prescan.captures
  in
  ...
  let tail_codomain =
    match output with
    | None -> codomain
    | Some (generator, _) -> list_tail_sort index (snd generator).note
  in
  let empty_sources =
    List.map (fun (_, source) -> empty_for_typ index source.note) generators
  in
  let empty_result =
    match output with
    | None -> Const "true"
    | Some (generator, _) -> empty_for_typ index (snd generator).note
  in
  let base count index =
    Eq (call name (arguments count index empty_sources), empty_result, [])
  in
  let source_patterns iter =
    source_arguments index iter generators heads tails
  in
  let extend result =
    match output with
    | None -> result
  ...
  | List1, _ ->
      let tail_declaration =
        premise_helper_declaration tail_name tail_domain tail_codomain
      in

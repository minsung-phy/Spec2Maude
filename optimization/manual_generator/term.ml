...
let is_special_sequence_sort = function
  | "ValList" | "NeValList"
  | "InstrList" | "NeInstrList"
  | "RefList" | "NeRefList" -> true
  | _ -> false

let sequence_family = function
  | "ValList" | "NeValList" -> "val"
  | "InstrList" | "NeInstrList" -> "instr"
  | "RefList" | "NeRefList" -> "ref"
  | _ -> "generic"

let sequence_tail_sort = function
  | "NeValList" -> "ValList"
  | "NeInstrList" -> "InstrList"
  | "NeRefList" -> "RefList"
  | sort -> sort

let empty_for_sort sort =
  match sequence_family sort with
  | "val" -> Const "valNil"
  | "instr" -> Const "instrNil"
  | "ref" -> Const "refNil"
  | _ -> Const "eps"

let concat_for_sort sort left right =
  match sequence_family sort with
  | "val" -> app "valConcat" [left; right]
  | "instr" -> app "instrConcat" [left; right]
  | "ref" -> app "refConcat" [left; right]
  | _ -> app "_ _" [left; right]

let rec sequence_for_sort sort = function
  | [] -> empty_for_sort sort
  | [term] -> term
  | term :: terms -> concat_for_sort sort term (sequence_for_sort sort terms)

let sequence = sequence_for_sort "SpectecTerminals"
...
let as_sequence_element index typ term =
  match Prescan.sort_of_typ index typ |> sequence_family with
  | "val" -> app "valSeq" [term]
  | "instr" -> app "instrSeq" [term]
  | "ref" -> app "refSeq" [term]
  | _ ->
...
let from_sequence_element index typ term =
  match Prescan.sort_of_typ index typ |> sequence_family with
  | "val" -> app "unvalSeq" [term]
  | "instr" -> app "uninstrSeq" [term]
  | "ref" -> app "unrefSeq" [term]
  | _ ->
      if Prescan.sort_of_typ index typ = "SpectecTerminals"
      then app "unseq" [term]
      else term

let sequence_operator index typ generic val_name instr_name ref_name =
  match Prescan.sort_of_typ index typ |> sequence_family with
  | "val" -> val_name
  | "instr" -> instr_name
  | "ref" -> ref_name
...
and translate_exp index exp =
  ...
  | CompE (left, right) ->
      let operator =
        ...
        | Prescan.SequenceComposition ->
            sequence_operator index exp.note
              "_++_" "valAppend" "instrAppend" "refAppend"
  ...
  | ListE exps ->
      ...
      |> List.map (fun exp ->
           translate_exp index exp |> as_sequence_element index exp.note)
      |> sequence_for_sort (Prescan.sort_of_typ index exp.note)
      |> fun terms ->
           app
             (sequence_operator index exp.note
                "`[_`]" "valList" "instrList" "refList")
             [terms]

  | LiftE inner ->
      app
        (sequence_operator index exp.note
           "lift" "valLift" "instrLift" "refLift")
        [translate_exp index inner]

  | MemE (element, collection) ->
      app
        (sequence_operator index collection.note
           "_<-_" "valOccurs" "instrOccurs" "refOccurs")
        [translate_exp index element; translate_exp index collection]
      |> wrap exp.note

  | LenE sequence ->
      app
        (sequence_operator index sequence.note
           "|_|" "valSize" "instrSize" "refSize")
  ...
  | IdxE (sequence, element_index) ->
      app
        (sequence_operator index sequence.note
           "_`[_`]" "valIndex" "instrIndex" "refIndex")
  ...
  | SliceE (sequence, start, length) ->
      app
        (sequence_operator index sequence.note
           "_`[_:_`]" "valSlice" "instrSlice" "refSlice")
        [ translate_exp index sequence
  ...
  | CallE (id, args) ->
      ...
      | None ->
          let name = Prescan.def_name index id in
          let translated = List.map (translate_arg index) args in
          begin match name, Prescan.sort_of_typ index exp.note, translated with
          | "concat-", _, [Const "val"; lists]
          | "concat-", "ValList", [_; lists] -> app "valFlatten" [lists]
          | "concat-", _, [Const "instr"; lists]
          | "concat-", "InstrList", [_; lists] -> app "instrFlatten" [lists]
          | "concat-", _, [Const "ref"; lists]
          | "concat-", "RefList", [_; lists] -> app "refFlatten" [lists]
          | _ -> app name translated
          end
      end
  ...
  | SubE (inner, source, target) ->
      let source_sort = Prescan.sort_of_typ index source in
      let target_sort = Prescan.sort_of_typ index target in
      let scalar_subsort =
        source_sort = target_sort
        || target_sort = "SpectecTerminal"
           && List.mem source_sort ["Num"; "Vec"; "Ref"; "Val"; "Instr"]
        || List.mem source_sort ["Num"; "Vec"; "Ref"]
           && List.mem target_sort ["Val"; "Instr"]
      in
      if source_sort = "Val" && target_sort = "Instr" then
        app "valToInstr" [translate_exp index inner]
      else if source_sort = "ValList" && target_sort = "InstrList" then
        app "valsToInstrs" [translate_exp index inner]
      else if Prescan.same_representation index source target || scalar_subsort then
        translate_exp index inner
      else
        invalid_arg
...
and translate_select index base path =
  ...
  | IdxP (parent, element_index) ->
      app
        (sequence_operator index parent.note
           "_`[_`]" "valIndex" "instrIndex" "refIndex")
...
and translate_update index base path replacement =
  ...
  | IdxP (parent, element_index) ->
      ...
      let updated_parent =
        app
          (sequence_operator index parent.note
             "_`[_=_`]" "valSetAt" "instrSetAt" "refSetAt")
  ...
  | SliceP (parent, start, length) ->
      ...
      let updated_parent =
        app
          (sequence_operator index parent.note
             "_`[_:_=_`]" "valSplice" "instrSplice" "refSplice")
          [ parent_value
          ; translate_exp index start
          ; translate_exp index length
          ; replacement
...
and translate_extension index base path extension =
  ...
  | RootP ->
      app
        (sequence_operator index path.note
           "_++_" "valAppend" "instrAppend" "refAppend")
        [base; extension]
  ...
  | IdxP (parent, element_index) ->
      ...
      let extended_parent =
        app
          (sequence_operator index parent.note
             "_`[_=++_`]" "valExtendAt" "instrExtendAt" "refExtendAt")
  ...
  | SliceP (parent, start, length) ->
      ...
      let extended_parent =
        app
          (sequence_operator index parent.note
             "_`[_:_=++_`]" "valExtendSlice" "instrExtendSlice" "refExtendSlice")
          [ parent_value
          ; translate_exp index start
          ; translate_exp index length
          ; extension
...
let translate_typ_conditions index value typ =
  ...
  | _, IterT (element_typ, iter) ->
      Iter.translate_conditions
        (translate_exp index) value (translate_check_typ index element_typ)
        (translate_sort index typ) iter
  | _, _ ->

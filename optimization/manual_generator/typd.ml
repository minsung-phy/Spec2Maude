...
let translate_constructor index id target case_conditions mixop typ =
  ...
  let declaration =
    let codomain =
      match Prescan.annotated_sort id.it with
      | Some sort -> sort
...
let translate_typcase index id target instance_conditions bound
  ...
  else translate_constructor index id target case_conditions mixop typ
...
let translate_variant index id params quants args cases =
  ...
  cases
  |> List.concat_map
       (translate_typcase index id target instance_conditions bound)

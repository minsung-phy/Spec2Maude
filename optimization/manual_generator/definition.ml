...
let inverse_pattern_variable variable =
  match variable.sort with
  | "Val" -> Some ("Instr", "instrToVal")
  | "ValList" -> Some ("InstrList", "instrsToVals")
  | "NeValList" -> Some ("NeInstrList", "instrsToNeVals")
  | _ -> None

let rec lower_instr_pattern inside = function
  | Var variable as original when inside ->
      begin match inverse_pattern_variable variable with
      | None -> original, []
      | Some (sort, inverse) ->
          let replacement =
            generated_variable (variable.name ^ "-AS-INSTR") sort
          in
          ( Var replacement
          , [MatchCond (original, App (inverse, [Var replacement]))] )
      end
  | (Var _ | Const _) as term -> term, []
  | App (name, args) ->
      let inside = inside || name = "instrConcat" || name = "instrList" in
      let args, guards =
        args
        |> List.map (lower_instr_pattern inside)
        |> List.split
      in
      App (name, args), List.concat guards

let lower_eq_condition = function
  | MatchCond (pattern, subject) ->
      let pattern, guards = lower_instr_pattern false pattern in
      MatchCond (pattern, subject) :: guards
  | (EqCond _ | MembershipCond _ | BoolCond _) as condition -> [condition]

let lower_rule_condition = function
  | EqCondition condition ->
      List.map (fun condition -> EqCondition condition)
        (lower_eq_condition condition)
  | RewriteCond (call, pattern) ->
      let pattern, guards = lower_instr_pattern false pattern in
      RewriteCond (call, pattern)
      :: List.map (fun condition -> EqCondition condition) guards

let lower_instr_val_patterns = function
  | Mb (term, sort) ->
      let term, guards = lower_instr_pattern false term in
      begin match guards with
      | [] -> Mb (term, sort)
      | guards -> Cmb (term, sort, guards)
      end
  | Cmb (term, sort, conditions) ->
      let term, guards = lower_instr_pattern false term in
      Cmb
        ( term, sort
        , guards @ List.concat_map lower_eq_condition conditions )
  | Eq (left, right, attrs) ->
      let left, guards = lower_instr_pattern false left in
      begin match guards with
      | [] -> Eq (left, right, attrs)
      | guards -> Ceq (left, right, guards, attrs)
      end
  | Ceq (left, right, conditions, attrs) ->
      let left, guards = lower_instr_pattern false left in
      Ceq
  ...
  | Crl (label, left, right, conditions) ->
      ...
      Crl
        ( label, left, right
        , List.map (fun guard -> EqCondition guard) guards
          @ List.concat_map lower_rule_condition conditions )
  | (SortDecl _ | SubsortDecl _ | VarDecl _ | OpDecl _) as statement ->
      statement
...
let translate_script script =
  ...
  let statements =
    ...
    @ Reld.optimization_statements ()
    |> List.map lower_instr_val_patterns
  in

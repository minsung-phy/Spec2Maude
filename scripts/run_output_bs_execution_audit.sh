...
ARTIFACT_DIR="$ROOT_DIR/artifacts/output-bs-execution-audit-$STAMP"
...
echo "[3/6] Strict structural invariants"
{
  echo "eq/ceq-valid:"
  ...
  echo
  echo "label step-from-step-pure count:"
  grep -n "step-from-step-pure-" output_bs.maude | wc -l
  echo
  echo "validation infer op count:"
  grep -n 'op \$infer-' output_bs.maude | wc -l
  echo
  echo "validation exec bridge count:"
  grep -n '\$exec-' output_bs.maude | wc -l
  echo
  echo "validation exec tail-empty count:"
  grep -n "exec-tail-empty" output_bs.maude | wc -l
  echo
...
echo "[4/6] Run isolated concrete Maude probe matrix"
...
echo "[6/6] Classify Maude warnings"

...
> 작성일: 2026-04-17 (교수님 미팅 피드백 반영 개정)
...
### 1.1.1 이 문서에서 먼저 잡아야 할 관점

교수님 미팅에서 반복해서 강조된 포인트는 다음 두 가지다.

1. **SpecTec 원문 구조와의 대응을 보존할 것**: 번역 결과가 원문 규칙의 모양을 잃지 않게 설명해야 한다.
2. **실행 문맥(context) 처리는 별도 계층으로 분리할 것**: 핵심 계산 규칙과 문맥 이동(heating/cooling)을 문서에서 명확히 분리해야 한다.

즉, 이 문서는 "규칙 자체"와 "규칙이 어디에서 발화되는지"를 분리해서 읽어야 한다.
...
프리스캔은 번역 전에 AST를 한 번 훑어, **필요한 선언을 미리 모아 선언 누락으로 인한 파싱 실패를 막기 위한 단계**다.

프리스캔이 모으는 것(핵심만):

1. `tokens`: 식/패턴에서 본 일반 토큰 이름
2. `ctors`: `VariantT` 케이스 이름 (토큰 중복 제거용)
3. `calls`: 선언 없이 호출된 함수 `(이름, 인자 수)`
4. `bool_calls`: `if`/`and`/`or`/`not` 안에서 호출된 함수
5. `dec_funcs`: 이미 `DecD`로 선언된 함수

이 정보는 바로 다음처럼 사용된다.

1. `tokens - ctors`를 `ops ... : -> WasmTerminal [ctor] .`로 일괄 선언
2. `calls` 중 `dec_funcs`를 제외해 helper `op` 자동 선언
3. `bool_calls`에 포함된 함수는 helper 선언 시 반환 타입을 `Bool`로 지정

즉 프리스캔은 "무엇을 선언해야 하는지"를 미리 정리하는 단계이고, 실제 `CTOR...`, `mb/cmb`, `eq/ceq`, `rl/crl` 생성은 2단계 변환에서 한다.
...
1. 방정식 안에서 `CTOR<name>A<arity>` 형태의 이름을 찾아, 선언이 빠진 생성자 `op`가 있으면 자동으로 보충한다.
2. 같은 이름에 대해 "인자 없는 타입 생성자"와 "인자 있는 타입 생성자"가 같이 있으면, 인자 없는 쪽을 제거해 파서 경고(`multiple distinct parses`)를 줄인다.
3. 같은 이름이 `WasmTerminal` 상수(`op`)와 변수(`var`)로 동시에 쓰이면, 상수 `op`를 제거해 이름 충돌(op/var ambiguity)을 막는다.
...
표현식을 변환할 때는 문자열 하나만 반환하지 않고, `texpr`를 반환한다.

- **`text`**: 변환된 Maude 코드 문자열 (예: `( A + B )`)
- **`vars`**: 그 표현식 안에서 발견한 변수 이름 목록 (예: `A`, `B`)

왜 이렇게 하냐면, **하위 표현식에서 찾은 변수 정보를 부모 표현식이 잃지 않게 하려고**다. 즉 전역 변수에 따로 쌓아두지 않고, 각 변환 함수가 자기 결과(`text`)와 변수 목록(`vars`)을 함께 들고 올라온다.

아래 헬퍼 함수(콤비네이터)는 이 `texpr`들을 합칠 때 쓴다.

```ocaml
tconcat : string → texpr list → texpr       (* 여러 text를 구분자로 연결하고 vars도 합침 *)
tmap    : (string → string) → texpr → texpr (* vars는 그대로 두고 text만 바꿈 *)
tjoin2  : (string → string → string) → texpr → texpr → texpr  (* 두 식 합치기 *)
tjoin3  : ... → texpr → texpr → texpr → texpr                 (* 세 식 합치기 *)
```

> **짧은 예시 : `BinE(AddOp, VarE "a", VarE "b")`**
>
> 1. `VarE("a")` → `{ text = "A"; vars = ["A"] }`
> 2. `VarE("b")` → `{ text = "B"; vars = ["B"] }`
> 3. 부모에서 `tjoin2`로 합침 → `{ text = "( A + B )"; vars = ["A"; "B"] }`
>
> 즉, `text`는 코드가 되고 `vars`는 변수 목록이 된다.
...
### 1.6 Step 계열과 Heating/Cooling 분담

교수님 미팅에서 나온 질문인 "Step, Step_pure, Step_read, Steps가 각각 무엇을 의미하는가"를 구현 기준으로 정리하면 아래와 같다.

| 항목 | 의미 | 번역/구현 위치 |
|---|---|---|
| `Step` | `config ~> config` 단일 실행 단계 | `translate_step_reld`가 `rl/crl` 생성 |
| `Step_pure` | 상태를 바꾸지 않는 명령열 단계 (`instr* ~> instr*`) | `translate_step_reld`가 `rl/crl` 생성 |
| `Step_read` | 상태를 읽어 결과 명령열을 만드는 단계 (`config ~> instr*`) | `translate_step_reld`가 `rl/crl` 생성 |
| `Steps` | reflexive-transitive closure (`~>*`) 성격의 반복 실행 | 주로 런타임 계층(`steps(...)`)에서 소화 |

또한 Step 의미론은 2계층으로 동작한다.

1. **핵심 실행 규칙 계층**: `output.maude`의 `rl/crl step(<...>) => <...>`
2. **문맥 이동 계층(heating/cooling)**: `wasm-exec.maude`의 `exec-label/frame/handler`, `restore-*`, `heat-*`, `cool-*`

이 분리는 "원문 규칙의 모양 보존"과 "실행 가능한 top-position 재작성"을 동시에 만족시키기 위한 설계다.
...
`Step`, `Step-pure`, `Step-read` 관계는 Bool predicate (`R(args)=true`)로 번역하지 않는다. 대신 `step` 항을 직접 재작성하는 `rl/crl` 규칙을 만든다.

핵심 의도는 다음과 같다.

1. **의미론적 구분 유지**: typing/validation은 판정(Bool), step 계열은 상태 전이(rewrite).
2. **top-position 실행 보장**: 실제 실행은 `step(<...>)` 꼭대기에서만 진행되도록 구성.
3. **문맥 규칙 분리**: RulePr 기반 브리지/컨텍스트 규칙은 여기서 직접 내리지 않고, heating/cooling 계층에서 처리.
...
[[ RelD(id="Step" | "Step-pure" | "Step-read", _, _, rules) ]] =>
    ...
    rules 내의 각 RuleD(case_id, binders, _, conclusion, prems) (rule_idx 번째) 에 대해 :
        ...
        --- 브리지/컨텍스트 규칙은 skip
        --- (문맥 이동은 wasm-exec.maude 의 heating/cooling 계층이 담당)
        ...
        [r]l / [c]rl step(< z_in | lhs_t is_var >) => < z_out | rhs_t is_var > [if COND] .
```

여기서 `is_var` 는 각 규칙마다 **고유** 한 `WasmTerminals` 변수로, assoc-tail 역할을 한다. 동일한 이름이 다른 관계의 `step` 규칙에서 재사용되면 coherence 문제가 생길 수 있으므로 `{REL}-{CASE}-IS` 형태의 고유 네이밍을 강제한다.
...
> ```
>
> **translator 출력(핵심 계산만):**
> ```maude
> crl [step-pure-binop-val] : step(< STEP-PURE-BINOP-VAL-Z |
...
> ```
>
> **wasm-exec의 heating/cooling(문맥 이동 담당):**
> ```maude
...
> ```
>
> 위처럼 "계산 규칙"과 "문맥 이동 규칙"을 분리하면, SpecTec 규칙 대응이 보존되고 실행 계층도 안정화된다.
...
| 실행 의미론 (Step-pure) | `rule Step_pure/c:`<br>`lhs ~>_pure rhs` | `RelD("Step-pure", ..., [RuleD("c", ..., TupE[lhs, rhs], prems)])` | `vars STEP-PURE-C-Z : WasmTerminal .`<br>`var  STEP-PURE-C-IS : WasmTerminals .`<br>`[r]l / [c]rl step(< Z \| LHS IS >) => < Z \| RHS IS > [if COND] .` |
| 실행 의미론 (Step-read) | `rule Step_read/c:`<br>`z;lhs ~>_read rhs` | `RelD("Step-read", ..., [RuleD("c", ..., TupE[<z ; lhs>, rhs], prems)])` | `[r]l / [c]rl step(< Z \| LHS IS >) => < Z \| RHS IS > [if COND] .` |
| 실행 의미론 (Step) | `rule Step/c:`<br>`z;lhs ~> z';rhs` | `RelD("Step", ..., [RuleD("c", ..., TupE[<z ; lhs>, <z' ; rhs>], prems)])` | `[r]l / [c]rl step(< Z \| LHS IS >) => < Z' \| RHS IS > [if COND] .` |
| 반복 실행 (Steps) | `relation Steps: config ~>* config` | (직접 RuleD를 Bool로 매핑하지 않음) | 런타임 계층의 `steps(...)`/반복 재작성으로 소화 |
...
**Maude:** (translate_step_reld → step 재작성 규칙)
```maude
vars STEP-PURE-NOP-Z : WasmTerminal .
var  STEP-PURE-NOP-IS : WasmTerminals .
rl [step-pure-nop] : step(< STEP-PURE-NOP-Z | CTORNOPA0 STEP-PURE-NOP-IS >)
...
**Maude:**
```maude
crl [step-read-local-get] : step(< STEP-READ-LOCAL-GETR-Z | CTORLOCALGETA1(STEP-READ-LOCAL-GETR-X) STEP-READ-LOCAL-GET-IS >)
...
```maude
--- 자동 생성 (참고용)
crl [step-local-set] : step(< STEP-LOCAL-SET-Z | STEP-LOCAL-SETP-VAL CTORLOCALSETA1(STEP-LOCAL-SETP-X) STEP-LOCAL-SET-IS >)
...
--- wasm-exec.maude 의 수작업 우회 (translate_step_reld 산출물이 아니라 수작업 eq/ceq)

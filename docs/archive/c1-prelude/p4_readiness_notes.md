...
6. Wasm-specific footer helpers
   ...
   - `$is-spectec-val-seq`, which is now generated from the source `val`
     ...
     for non-Wasm specs;
   - `CTORFRAMEA2`, currently generated by a Wasm frame-record special case
     over fields `LOCALS` and `MODULE`;
...
- benchmark harness;
- Wasm frame/store representation helpers and hardcoded frame-record lowering;

...
2. 해당 syntax가 elaborated IL에서 `*` 또는 `+`로 사용됐는지 정적분석한다.
3. 사용된 경우 Maude Prelude의 `LIST{X :: TRIV}`를 이용해 전용 리스트를 만든다.
4. 기존의 일반적인 `x*`도 수동 `SpectecTerminals` 구현 대신 Prelude 리스트를 사용한다.

이 문서는 sort/list 표현 단계만 다룬다. `heatingTarget`, hole operator,
heating/cooling 및 `Step/ctxt-instrs` focus 탐색은 후속 단계다. 따라서 이 문서의
작업만 끝났다고 timeout 해결을 주장하면 안 된다.
...
새 hint는 `maude_sort` 하나만 사용한다.

```spectec
hint(maude_sort)
```

`hint(maude_list)`와 `hint(maude_list_subsort)`는 만들지 않는다.
...
`hint(maude_sort)`와 정적분석 결과를 이용해 `output.maude` 앞부분에 생성한다.
...
```spectec
syntax num hint(desc "number value") hint(maude_sort) =
  | CONST numtype num_(numtype)

syntax vec hint(desc "vector value") hint(maude_sort) =
...
2. hint expression이 `El.Ast.SeqE []`인지 검증한다.
3. 중복 hint, 알 수 없는 target, parameterized syntax의 지원 범위를 검증한다.
4. `has_typ_hint`와 `maude_sort_types` accessor를 제공한다.

`maude_sort`가 없는 syntax 이름을 코드에서 `val`, `instr` 문자열로 hard-code하지
않는다.

완료 조건:

- `val`, `instr`의 hint가 IL `TypH`에서 발견됨
- hint를 제거하면 해당 sort/list 요청도 사라짐
...
Typed list를 도입해도 현재 `Step/ctxt-instrs`의 전체 timeout이 자동으로 해결됐다고
볼 수 없다. 특히 `instr* instr_1*`는 동일한 `InstrList`이므로 focus 경계 탐색 문제가
남는다.

다음 단계에서는 별도 계획으로 다음을 구현하고 검증한다.

```text
각 direct step rule의 enabledness helper
heatingTarget/focus discovery
ValList와 InstrList를 별도 인자로 보관하는 hole
heating/fire/cooling
동일 benchmark의 실행시간과 rewrite 수 비교
```

이 후속 단계에서도 `ValList < InstrList`를 추가해서 flat sequence를 복구하지 않는다.

...
The `baseline2` branch uses ordinary untyped lists and direct context rules.
The sequence backend uses single-element recursion without memoization,
cursors, compressed runs, or batched traversal. See [BASELINE.md](BASELINE.md)
for the comparison boundary and focused verification scope.
